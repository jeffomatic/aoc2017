let edge_size target =
  let rec aux target prev_size prev_max =
    ...
    if target < new_max then
      new_size, prev_max
    else
      aux target new_size new_max
  in
  aux target 1 1
...
let dxdy target =
  ...
  let ll = ul + (size - 1) in
  if target <= ur then
    half_size, - (half_size) + (target - prev_max)
  else if target <= ul then
    half_size - (target - ur), half_size
  else if target <= ll then
    -half_size, half_size - (target - ul)
  else
    -half_size + (target - ll), -half_size
