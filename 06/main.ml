...
let redistribute state from_index =
  ...
  let rec aux state blocks next_index =
    if blocks = 0 then
      ()
    else if (Array.length state) = next_index then
      aux state blocks 0
    else (
...
let () =
  ...
  let rec aux state seen steps =
    if StringSet.mem (serialize state) seen then
      steps
    else (
